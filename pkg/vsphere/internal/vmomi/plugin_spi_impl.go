...
func (spi *PluginSPIImpl) DeleteMachine(ctx context.Context, machineName string, providerID string, providerSpec *api.VsphereProviderSpec1, secrets *corev1.Secret) (string, error) {
	client, err := createVsphereClient(ctx, secrets)
	if err != nil {
		return "", err
	}
...
func (spi *PluginSPIImpl) ShutDownMachine(ctx context.Context, machineName string, providerID string, providerSpec *api.VsphereProviderSpec1, secrets *corev1.Secret) (string, error) {
	client, err := createVsphereClient(ctx, secrets)
	if err != nil {
		return "", err
	}
...
func (spi *PluginSPIImpl) GetMachineStatus(ctx context.Context, machineName string, providerID string, providerSpec *api.VsphereProviderSpec1, secrets *corev1.Secret) (string, error) {
	client, err := createVsphereClient(ctx, secrets)
	if err != nil {
		return "", err
	}
